...
namespace Standalone
{
    public class BordsWETold
    {
        ...
        private VariabelSamlingBordOld bord;
        private VariabelSamlingBildOld bild;
        ...
        public void run()
        {
            bord = new VariabelSamlingBordOld();
            bild = new VariabelSamlingBildOld(strukturset);
            ...
            if (Hittabord())
            {
                ...
                double WET_L = Sidoraknaren(1);
                double WET_R = Sidoraknaren(-1);

                if (WET_L != 0)
                    WETstring = "WET vänster = " + WET_L.ToString("0.00") + " cm";

                if (WET_R != 0)
                    WETstring += "\n" + "WET höger = " + WET_R.ToString("0.00") + " cm";

                double WET_C = GetWET(0, 215) / 10;
                //MessageBox.Show(resultat.ToString());
                //MessageBox.Show(bord.bord_c[0].ToString() + "\n" + bord.bord_c[1].ToString() + "\n" + bord.bord_c[2].ToString() + "\n");

                if (WET_C != 0)
                    WETstring += "\n" + "WET center = " + WET_C.ToString("0.00") + " cm";

                if (WET_L > 0 && WET_L <= 1.1 || WET_R > 0 && WET_R <= 1.1 || WET_C > 0 && WET_C <= 1.1)
                    MessageBox.Show(":( \n \nMinst ett WET-värde är under 1.1 cm, kontrollera att bordet inte ligger för långt upp");
                else if (WET_L > 1.2 || WET_R > 1.2 || WET_C > 1.2)
        ...
        private double Sidoraknaren(int sida)
        {
            double WET = 0;
            int dist = 215;
            //string distlist = "";
        ...
        public int Pixel(int[,] matris, double lat_flytt)
        {
            ...
            {
                bord_kant = n - 3; // Härifrån börjar vi räkna WET (mitten av pixeln?)
            }
            return bord_kant;
        }

        public double GetWET(int sida, int dist)
        {
            int[,] matris = GetMatris(bord.bord_c[2]);
            ...
            else
            {
                ...
                for (int j = bord_kant; j <= bord_kant + Math.Round(70 / bild.image_res[1]) || j < bild.image_size[1]; j++)
                {
                    for (int k = 0; k < steg; k++)
                    {
                        ...
                        {
                            WET += HUtoRelStopPwr(-824) * bild.image_res[1] * (1 / steg);
                            //resultat2 += "\n" + "WET = " + HUtoRelStopPwr(-824) * bild.image_res[1] * (1 / steg) + ", HU = " + strukturset.Image.VoxelToDisplayValue(matris[bord.bord_c_pixel[0], j]);
                        }
        ...
        public void ReadCalibrationCurve()
        {
            ...
            //MessageBox.Show(hjarna.ToString());
    ...
    public class VariabelSamlingBordOld
    {
        public VariabelSamlingBordOld()
        {
        ...
        public void DefinieraBordCenter(VariabelSamlingBildOld bild)
        {
    ...
    public class VariabelSamlingBildOld
    {
        public VariabelSamlingBildOld(StructureSet Strset)
        {
            strukturset = Strset;
            Definierabild();
        }
